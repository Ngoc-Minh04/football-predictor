...
async function optimize() {
  ...
  // Chúng ta sẽ quét qua các trọng số blend khác nhau
  const blendWeights = [
    { useDC: false, mleWeight: 0, rollingWeight: 0.55, xgWeight: 0.20 }, // Baseline không DC
    { useDC: true, mleWeight: 0.10, rollingWeight: 0.70, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.15, rollingWeight: 0.65, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.20, rollingWeight: 0.60, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.25, rollingWeight: 0.55, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.30, rollingWeight: 0.50, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.35, rollingWeight: 0.45, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.40, rollingWeight: 0.40, xgWeight: 0.20 }, // Mặc định hiện tại
    { useDC: true, mleWeight: 0.45, rollingWeight: 0.35, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.50, rollingWeight: 0.30, xgWeight: 0.20 },
    { useDC: true, mleWeight: 0.60, rollingWeight: 0.20, xgWeight: 0.20 },
  ];

  // Disable console.log completely for predictions
  console.log = () => {};

  const results = [];

  for (const opt of blendWeights) {
    let correctScore = 0;
    let correct1X2 = 0;
    let correctOU = 0;
    ...
    for (const data of matchDataList) {
      const pred = predict({
        ...
        isNeutral: false,
        dixonColesStrengths: opt.useDC ? data.dixonColesStrengths : null,
        mleWeight: opt.mleWeight,
        rollingWeight: opt.rollingWeight,
        xgWeight: opt.xgWeight,
      });
      ...
      // Always apply IPF for these tests
