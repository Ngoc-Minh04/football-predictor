...
async function optimize() {
  ...
  originalLog('Finished pre-fetching data. Starting Stage 1: Sweep lambda3 & nbR');

  // Mute logs
  console.log = () => {};

  // Giai đoạn 1: Quét lambda3 và nbR (cố định các tham số khác ở mức mặc định)
  const lambda3Vals = [0.02, 0.05, 0.08, 0.11, 0.14, 0.17, 0.20];
  const nbRVals = [2.0, 3.0, 4.5, 6.0, 8.0, 11.0, 15.0];

  let bestScoreStage1 = -1;
  let bestLambda3 = 0.11;
  let bestNbR = 8.0;
  ...
  for (const l3 of lambda3Vals) {
    for (const r of nbRVals) {
      ...
      for (const data of matchDataList) {
        ...
        if (data.m.score_home === pred.score.home && data.m.score_away === pred.score.away) {
          correctScore++;
        }
      }
  ...
  originalLog(`Stage 1 Finished. Best Score: ${bestScoreStage1}/50 (${(bestScoreStage1/50*100).toFixed(1)}%). Best lambda3: ${bestLambda3}, Best nbR: ${bestNbR}`);

  // Giai đoạn 2: Quét bivariateWeight, drawBoostMax, rhoVal (cố định bestLambda3 và bestNbR)
  originalLog('Starting Stage 2: Sweep bivariateWeight, drawBoostMax & rhoVal');
  console.log = () => {};

  const bivariateWeights = [0.50, 0.70, 0.80, 0.90, 0.95, 1.00];
  const drawBoostMaxs = [0.0, 0.10, 0.20, 0.28, 0.35, 0.45];
  const rhoVals = [undefined, -0.02, -0.06, -0.10, -0.14];
  ...
  stage2Results.sort((a, b) => b.correctScore - a.correctScore);
  
  console.log('Top 15 Stage 2 configurations:');
  console.table(stage2Results.slice(0, 15));
}
